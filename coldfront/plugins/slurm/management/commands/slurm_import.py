...
class Command(BaseCommand):
    ...
    def coldfront_get_allocation_attribute(self, allocation, allocation_attribute_type, value):
        """
    ...
    def coldfront_assign_allocation_resource(self, allocation, alloc_kwargs):
        ...
        if allocation is None:
            logger.warn(f"NOOP - add Resource {self.resource} to Allocation {alloc_kwargs}")
            return
    ...
    def import_to_coldfront(self, slurm_cluster):
        ...
        for name, account in slurm_cluster.accounts.items():
            ...
            for uid, user in account.users.items():
                if uid == "root":
                    continue
                qos_spec = next(filter(lambda spec: spec.startswith("QOS"), user.spec_list()))
                user, project, allocation = self.add_user(uid, name, slurm_cluster.name, qos_spec)
    ...
    def handle(self, *args, **options):
        ...
        try:
            self.acct_name_attr = AllocationAttributeType.objects.get(name=SLURM_ACCOUNT_ATTRIBUTE_NAME)
        ...
        try:
            self.user_specs_attr = AllocationAttributeType.objects.get(name=SLURM_USER_SPECS_ATTRIBUTE_NAME)
        except AllocationAttributeType.DoesNotExist:
            logger.error(
                "No AllocationAttributeType '%s' found in ColdFront",
                SLURM_ACCOUNT_ATTRIBUTE_NAME,
